...
const PatientDashboard = ({ user }) => {
  ...
  const fetchSurveys = async () => {
    try {
      setFetchError(null);
      const response = await axios.get(`http://localhost:8080/api/researcher/surveys/active/${user.username}?t=${new Date().getTime()}`, {
  ...
  const handleSubmit = async (e) => {
    ...
    setSubmitting(true);
    try {
      await axios.post('http://localhost:8080/api/researcher/surveys/submit', {
